...
class ParkingListTabViewController : UITableViewController {
    ...
    override func tableView(tableView: UITableView, cellForRowAtIndexPath indexPath: NSIndexPath) -> UITableViewCell {
        ...
        let cell2 = tableView.dequeueReusableCellWithIdentifier("parkingSpotCell", forIndexPath: indexPath) as! parkingSpotTableViewCell
        
        //cell2.textLabel?.text = parkingDecks[indexPath.row].name
        
        
        
        cell2.parkingSpotNameLabel.text = parkingDecks[indexPath.row].name
        
        cell2.parkingSpotsLeft.text?.appendContentsOf(String(parkingDecks[indexPath.row].parkingSpotsLeft))
    ...
    override func tableView(tableView: UITableView, didSelectRowAtIndexPath indexPath: NSIndexPath) {
        ...
        if let tabBarController = storyboard!.instantiateViewControllerWithIdentifier("TabBarViewController") as? UITabBarController {
            self.presentViewController(tabBarController, animated: true, completion: nil)
            tabBarController.selectedIndex = 1 //switch tabs
        }
